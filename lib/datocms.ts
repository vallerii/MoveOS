 ...
 *
 * Requires DATOCMS_API_TOKEN (a DatoCMS "Content Delivery API" read-only
 * token — see .env.example). Every helper here fails soft: on a missing
 * token, network error, or GraphQL error it logs and returns an empty
 * result instead of throwing, so a CMS hiccup never takes the homepage
 * down — the trust section just renders fewer/no article cards.
 */
