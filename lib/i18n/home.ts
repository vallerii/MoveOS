...
export interface HomeCopy {
   ...
   * Expands on the existing situations.linksPanel grid (which stays
   * untouched) by spelling out, per pain, what the free help actually
   * covers. `items` is in PAIN_SLUGS order (deposit, admin, belongings,
   * urgent, buyout, repair) so it maps 1:1 to PAIN_ICONS/PAIN_SLUGS in the
   * component rather than needing its own per-pain keying.
   */
  included: {
    heading: string;
    subheading: string;
  ...
  ru: {
    ...
    included: {
      ...
      items: [
        {
          title: "Депозит",
          body: "80% людей теряют депозит при выезде. Проверяем акт и фото на въезд и выезд и подсказываем, как вернуть депозит полностью — без незаконных удержаний.",
        },
        {
          title: "Бытовые дела",
